...
//
// ----- alias for display-currency (BEC -> btc) -----
//   - 화면 설정에서 'BEC'를 선택하면 내부 포맷 타입을 'btc'로 통일
//   - 값 계산(곱셈/소수처리)은 그대로 유지하고, 표시 라벨만 BEC로 보정
//
...
function formatCurrencyAmountWithForcedDecimalPlaces(amount, formatType, forcedDecimalPlaces) {
	// BEC → btc 별칭 정규화
	const tAlias = _normType(formatType);

	// 정규화된 타입으로 조회 + 안전 폴백(BEC)
	let currencyType = global.currencyTypes[tAlias];
	if (currencyType == null) {
		const fallbackUnit = (coinConfig && coinConfig.defaultCurrencyUnit) ? coinConfig.defaultCurrencyUnit : null;
		if (!fallbackUnit) { throw `Unknown currency type: ${tAlias}`; }
		currencyType = {
			type: "native",
			name: fallbackUnit.name,           // "BEC"
			multiplier: fallbackUnit.multiplier,
			decimalPlaces: fallbackUnit.decimalPlaces
		};
	}
	...
	if (currencyType.type == "native") {
		...
		if (forcedDecimalPlaces >= 0) {
			...
			// btc 타입일 때 화면 라벨만 BEC로 강제
			const unitName =
				((tAlias === "btc") && coinConfig && coinConfig.defaultCurrencyUnit && coinConfig.defaultCurrencyUnit.name)
					? coinConfig.defaultCurrencyUnit.name
					: currencyType.name;

			return {val:baseStr, currencyUnit:unitName, simpleVal:baseStr, intVal:parseInt(dec)};
			...
			// btc 타입일 때 화면 라벨만 BEC로 강제
			if (tAlias === "btc" && coinConfig && coinConfig.defaultCurrencyUnit && coinConfig.defaultCurrencyUnit.name) {
				returnVal.currencyUnit = coinConfig.defaultCurrencyUnit.name;
			}
...
function formatCurrencyAmount(amount, formatType) {
	// 상위 래퍼에서도 타입 정규화
	formatType = _normType(formatType);
